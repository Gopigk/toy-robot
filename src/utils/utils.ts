...
export const isValidCoordinate = (x: number) =>
  Number.isInteger(x) && Math.sign(x) >= 0;

export const getCommandValues: GetCommandList = (command) =>
  command.split(/[\s,]+/);
...
export const getSquareSize: GetSquareSize = (type = "x") =>
  `${(1 / TABLE_DIMENSION[type]) * 100}%`;
...
export const createRobotTransition: CreateRobotTransition = (params) => {
  const { coordinate, prevCoordinate, prevIsPlaced, rotateDeg, prevRotateDeg } =
    params;
